...
@app.get("/api/v1/forecast/events")
async def get_weekly_events(
    lat: float, 
    lon: float
):
    """
    Récupère la chronologie des événements météo pour les 7 prochains jours.
    Utilisé par le frontend pour la Timeline et les prévisions détaillées.
    """
    # 1. Appel au service de base de données
    print(lat,lon)
    forecasts = await db_service.get_7day_forecast(lat, lon)
    
    # 2. Vérification si données présentes
    if not forecasts:
        raise HTTPException(
            status_code=404, 
            detail="Aucune donnée de prévision trouvée pour ces coordonnées. Vérifiez que Spark génère bien les données."
        )
    
    # 3. Retourne le format JSON structuré
    return {
        "status": "success",
        "latitude": lat,
        "longitude": lon,
